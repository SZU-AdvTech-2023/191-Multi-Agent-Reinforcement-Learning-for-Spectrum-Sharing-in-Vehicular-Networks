...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.height = height  # 环境长度

        self.V2Vchannels = V2Vchannels()  # V2V通道
        self.V2Ichannels = V2Ichannels()  # V2I通道
        self.vehicles = []  # 存储环境中所有车辆

        self.demand = []    # 每辆车的通信需求
        self.V2V_Shadowing = []
        self.V2I_Shadowing = []
        self.delta_distance = []    # 每个车辆的位移
        self.V2V_channels_abs = []  # V2V的通信绝对通道
        self.V2I_channels_abs = []  # V2I的通信绝对通道
        ...
        self.sig2 = 10 ** (self.sig2_dB / 10)   # 噪声功率

        self.n_RB = n_veh   # 资源块数量
        self.n_Veh = n_veh  # 车辆数量
        self.n_neighbor = n_neighbor    # 每个车辆的邻居数量
        self.time_fast = 0.001   # 快速更新的时间间隔
        self.time_slow = 0.1  # update slow fading/vehicle position every 100 ms 慢速更新的时间间隔
        self.bandwidth = int(1e6)  # bandwidth per RB, 1 MHz  每个资源块的带宽
        # self.bandwidth = 1500
        self.demand_size = int((4 * 190 + 300) * 8 * 4)  # V2V payload: 1060 Bytes every 100 ms 每个车辆的通信需求大小
        # self.demand_size = 20

        self.V2V_Interference_all = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2  # 所有V2V通信的干扰
    '''
    在无线通信中，通道的变化速度通常由用户的移动速度和无线信号的传播环境决定。根据变化的快慢，通道的变化可以分为慢衰落和快衰落。慢衰落（Slow Fading）主要是由于用户的大范围移动引起的，比如用户从一个街区移动到另一个街区，或者从室内移动到室外，这会导致通道的大尺度变化。慢衰落的变化速度相对较慢，
    因此更新频率可以较低。在这个代码中，慢衰落的更新间隔由 time_slow变量表示。快衰落（Fast Fading）主要是由于用户的小范围移动或者周围环境的微小变化引起的，比如用户在一个房间内移动，或者周围的人和物体的移动，这会导致通道的小尺度变化。快衰落的变化速度相对较快，因此需要更高的更新频率。在这个代码中，快衰落的更新间隔由 time_fast变量表示。
    在这个模拟环境中，time_slow和 time_fast变量用于控制慢衰落和快衰落的更新频率，以模拟真实的无线通信环境。
    '''
